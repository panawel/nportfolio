...
- The case-study "All Projects" link (`BackToProjects`) goes back in history when the page was reached from the homepage (`RouteMemory` in the root layout remembers the previous route, client navigations only), so the homepage returns to its exact scroll position; a plain `/#projects` link jumped to the top of the Projects section. Reached any other way (direct link, reload) it falls back to `/#projects`. The browser's own back button already restores scroll.
